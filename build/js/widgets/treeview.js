...
class Treeview {
  ...
  _setupListeners() {
    ...
    _autoOpenMenu() {
        ...
        const self = this
        const $navLinks = $('.nav-link', this._element)
        if (!$navLinks.length) {
            return
        }
        ...
        if ($navLinks.length > 0) {
            for(let i = 0; i < $navLinks.length; i++) {
                if ($navLinks[i].getAttribute('href') && href === $navLinks[i].href) {
                    activedElement = $navLinks[i]
                    break;
                }
            }
        }
        ...
        $(activedElement).addClass('active')
        $(activedElement).parents('.nav-treeview').each(function(){
            const parentLi = $(this).parent('.nav-item')
            self.expand($(this), parentLi)
        })
       // $($link).parents('.nav-treeview').show()
    }
